...
pub fn test_searches(
    ...
    let album_results = album::search_albums(client_ring.clone(), "Twin Fantasy")
        .expect("Error in album::search_albums");

    println!("Twin Fantasy albums:\n{:#?}", album_results.items[0]);

    let artist_results = artist::search_artists(client_ring.clone(), "Brockhampton")
        .expect("Error in artist::search_artists");

    println!("Brockhampton artists:\n{:#?}", artist_results.items[0]);

    let track_results = track::search_tracks(client_ring.clone(), "Shirim")
